...
public final class PrintHelper {
    ...
    private PrintHelper(final PrintStream ps, final String name, final String slot, final String targetName, final String targetSlot) {
        this.ps = ps;
        ...
        if (isModule) {
            if (JBossModule.MAIN_SLOT.equals(slot)) {
                ps.println(String.format("<module xmlns=\"urn:jboss:module:1.1\" name=\"%s\">", name));
            } else {
                ps.println(String.format("<module xmlns=\"urn:jboss:module:1.1\" name=\"%s\" slot=\"%s\">", name, slot));
            }
        } else {
            if (JBossModule.MAIN_SLOT.equals(targetSlot)) {
                ps.print(String.format("<module-alias xmlns=\"urn:jboss:module:1.1\" name=\"%s\" target-name=\"%s\"/>", name, targetName));
            } else {
                ps.print(String.format("<module-alias xmlns=\"urn:jboss:module:1.1\" name=\"%s\" target-name=\"%s\" target-slot=\"%s\"/>", name, targetName,
                        targetSlot));
            }
        }
    }
     ...
     * @return print helper.
     */
    public static PrintHelper moduleAlias(final PrintStream ps, final String name, final String slot, final String targetModule, final String targetSlot) {
